...
function expectInvalid(path: string) {
  return expectConfig(`__tests__/fixtures/config-invalid/${path}`).rejects.toThrow(/Config parse error:.+/);
}

function expectValid(path: string) {
  return expectConfig(`__tests__/fixtures/config-valid/${path}`).resolves.toBeTruthy();
}
