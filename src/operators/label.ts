...
import { addLabels, commitStatus, getLabels, postComment, removeLabels } from '../github';
...
export default async function (label: Label, commands: Commands): Promise<void> {
   ...
   */
  function needs() {
    const needCommands = [...commands.prefix(`/needs ${label.prefix}`), ...commands.prefix(`/need ${label.prefix}`)];
  ...
  function computeLabels() {
    ...
    const adding = commands
      .prefix(`/${label.prefix}`)
      .flatMap((add) =>
        add.args.filter((value) => label.list.includes(value)).map((value) => `${label.prefix}/${value}`),
      );
  ...
  if (isCreatedOpened() && isAuthorAssociationAllowed(label.author_association)) {
    computeLabels();
  }
