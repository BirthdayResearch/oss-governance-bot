...
async function parseLabel(capture: Capture, array: RegExpExecArray): Promise<string | undefined> {
  ...
  if (capture.github_release) {
    ...
    if (!(await hasReleaseByTag(`v${capturedText}`)) && !(await hasReleaseByTag(capturedText))) {
      return;
    }
  }
