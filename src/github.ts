...
export function initClient(token: string = core.getInput('github-token')): InstanceType<typeof GitHub> {
...
export async function commitStatus(
  ...
  if (github.context.payload.comment && github.context.payload.issue?.pull_request) {
